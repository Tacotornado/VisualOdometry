...
class LiveTelloVIO:
    ...
    def process_frame(self, frame, scale_factor, imu_R):
        ...
        # Calculate how far visual feature tracks physically shifted across the pixel grid
        pixel_displacements = np.linalg.norm(good_p1 - good_p2, axis=1)
        avg_pixel_move = np.mean(pixel_displacements)
        
        # Threshold Tuning: If tracking features shift less than 0.6 pixels on average, 
        # the frame changes are purely image sensor noise. Kill the movement scale completely.
...
class Esp32VideoSource(BaseVideoSource):
    """Parses incoming network stream variables from Seeed Studio XIAO ESP32-S3."""
...
def main():
    ...
    # Choose your active hardware mode here ("TELLO" or "ESP32")
    # -------------------------------------------------------------------------
    ACTIVE_MODE = "TELLO"  
    ...
    if ACTIVE_MODE == "TELLO":
        ...
        print("Configuring operational script environment for self-made custom ESP32 drone platform...")
        video_worker = Esp32VideoSource(ESP32_URL, frame_queue, stop_event)
    else:
        raise ValueError("Invalid target source specified. Assign either 'TELLO' or 'ESP32'.")
    ...
    try:
        while True:
            ...
            if ACTIVE_MODE == "TELLO" and drone:
                ...
                # Fallback metrics when running the custom ESP32 build without a data link
