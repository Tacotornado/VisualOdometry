...
# Import the Madgwick filter from the ahrs framework
...
class LiveTelloVIO:
    def __init__(self):
        ...
        # Initialize OpenCV ORB Detector (Max 500 feature points for real-time speed)
        self.orb = cv2.ORB_create(nfeatures=500, scaleFactor=1.2, nlevels=8, edgeThreshold=31)
        # Brute-Force Matcher using Hamming Distance for binary ORB descriptors
    ...
    def process_frame(self, frame, scale_factor, imu_R):
        ...
        # Match descriptors between current frame and previous frame
        matches = self.matcher.match(self.prev_des, des)
        
        # Sort matches based on distance (best matches first)
        matches = sorted(matches, key=lambda x: x.distance)
        
        # Safeguard: Ensure we have enough high-quality matches
        if len(matches) < 15:
            self.prev_frame = gray
            self.prev_kp = kp
            self.prev_des = des
        ...
        # Robustly calculate Essential Matrix via RANSAC geometric estimation
        ...
        # Extract translation direction vector (ignoring messy monocular cam rotation)
        _, _, cam_t, _ = cv2.recoverPose(E, good_p1, good_p2, self.K, mask=mask)
        
        # --- VIO FUSION UPDATE ---
        # Enforce the stabilized orientation from the Madgwick-filtered IMU matrix
        R = imu_R 
        ...
        # Render clean keypoint matches onto the tracking display window
        display_frame = frame.copy()
        mask_ravel = mask.ravel()
        for i, m in enumerate(matches[:50]): # Draw the top 50 matches for visualization clarity
            if i < len(mask_ravel) and mask_ravel[i]:
                pt2 = tuple(map(int, kp[m.trainIdx].pt))
                display_frame = cv2.circle(display_frame, pt2, 4, (0, 255, 0), -1)
...
def main():
    ...
    # --- SETUP INTERNALS FOR MADGWICK FILTER ---
    # Instantiate the Madgwick object with a standard operational frequency configuration
    madgwick_filter = Madgwick(frequency=30.0, gain=0.1)
    # Start tracking orientation state initialized as a standard identity quaternion
    ...
    try:
        while True:
            ...
            # --- SYNTHESIZE RAW IMU METRICS FOR MADGWICK ---
            roll, pitch, yaw = drone.get_roll(), drone.get_pitch(), drone.get_yaw()
            current_angles = np.array([roll, pitch, yaw])
            
            # Calculate angular velocity (Gyro scope imitation via Rad/Sec derivatives)
            angle_diff = np.radians(current_angles - prev_angles)
            # Handle standard wrapping boundary discontinuities (-180 to 180 transitions)
            angle_diff = (angle_diff + np.pi) % (2 * np.pi) - np.pi
            gyro_data = angle_diff / dt
            prev_angles = current_angles.copy()
            
            # Synthesize linear accelerometer metrics relative to gravity vectors
            rot_current = SciPyRot.from_euler('xyz', [roll, pitch, yaw], degrees=True)
            gravity_world = np.array([0.0, 0.0, 9.81])
            # Project global gravity down onto the local moving body coordinates
            accel_data = rot_current.inv().apply(gravity_world)
            
            # Pass metrics to the Madgwick Engine to step forward orientation estimates
            current_q = madgwick_filter.updateIMU(q=current_q, gyr=gyro_data, acc=accel_data)
            
            # Convert fused Madgwick quaternion directly back into our VIO rotation matrix
            fused_rot_obj = SciPyRot.from_quat([current_q[1], current_q[2], current_q[3], current_q[0]]) # SciPy format: (x,y,z,w)
            imu_R = fused_rot_obj.as_matrix()
            
            # --- SCALE EXTRACTION TELEMETRY ---
            ...
            print(f"ORB+Madgwick VIO -> X: {x:.2f}m, Y: {y:.2f}m, Z: {z:.2f}m", end="\r")
        ...
        if len(path_history) > 5:
            ...
            final_ax.set_title('Tello Trajectory - ORB + Madgwick Filter VIO Final Path', fontsize=14, pad=20)
