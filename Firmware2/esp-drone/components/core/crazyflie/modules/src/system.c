...
/*Dummy variable*/
static int bcLighthouse4 = 0;
static int bcZRanger = 0;
static int bcLoco = 0;
static int bcDWM1000 = 0;
...
void systemInit(void)
{
  ...
  wifilinkInit();
  DEBUG_PRINT_LOCAL("Wifi link init done");
  sysLoadInit();
  DEBUG_PRINT_LOCAL("Sys load init done");

  /* Initialized here so that DEBUG_PRINT (buffered) can be used early */
  debugInit();
  DEBUG_PRINT_LOCAL("Debug init done");
  crtpInit();
  DEBUG_PRINT_LOCAL("CRTP init done");
  consoleInit();
  DEBUG_PRINT_LOCAL("Console init done");
  ...
  configblockInit();
  DEBUG_PRINT_LOCAL("Config block init done");
  //storageInit();
  workerInit();
  DEBUG_PRINT_LOCAL("Worker init done");
  adcInit();
  DEBUG_PRINT_LOCAL("ADC init done");
  ledseqInit();
  DEBUG_PRINT_LOCAL("Led sequence init done");
  pmInit();
  DEBUG_PRINT_LOCAL("Debug print init done");
  buzzerInit();
  DEBUG_PRINT_LOCAL("Buzzer init done");
...
void systemTask(void *arg)
{
  ...
  systemInit();
  DEBUG_PRINTI("System initialized");
  commInit(); 
  DEBUG_PRINTI("Comm initialized");
  commanderInit();
  DEBUG_PRINTI("Commander initialized");

  StateEstimatorType estimator = anyEstimator;
  estimatorKalmanTaskInit();
  DEBUG_PRINTI("Kalman initialized");
  //deckInit();
  //estimator = deckGetRequiredEstimator();
  stabilizerInit(estimator);
  DEBUG_PRINTI("Stabilizer initialized");
  cameraStreamInit();
  ...
  //}
  soundInit();
  DEBUG_PRINTI("Sound initialized");
  memInit();
  DEBUG_PRINTI("Mem initialized");
...
PARAM_ADD(PARAM_INT8, bcZRanger, &bcZRanger)
//PARAM_ADD(PARAM_INT8, bcFlow, &bcFlow)
//PARAM_ADD(PARAM_INT8, bcLighthouse, &bcLighthouse)
PARAM_ADD(PARAM_INT8, bcLighthouse4, &bcLighthouse4)
PARAM_ADD(PARAM_INT8, bcLoco, &bcLoco)
//PARAM_ADD(PARAM_INT8, bcFlow, &bcFlow)
//PARAM_ADD(PARAM_INT8, bcFlow2, &bcFlow2)
PARAM_ADD(PARAM_INT8, bcDWM1000, &bcDWM1000)
